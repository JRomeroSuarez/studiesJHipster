  ...
  return (
    <div>
      <h2 id="answer-heading" data-cy="AnswerHeading">
        <Translate contentKey="studiesJHipsterApp.answer.home.title">Answers</Translate>
        <div className="d-flex justify-content-end">
          <Button className="mr-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="studiesJHipsterApp.answer.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to={`${match.url}/new`} className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="studiesJHipsterApp.answer.home.createLabel">Create new Answer</Translate>
          </Link>
        </div>
      </h2>
        ...
        {answerList && answerList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                <th>
                  <Translate contentKey="studiesJHipsterApp.answer.id">ID</Translate>
                </th>
                <th>
            ...
            <tbody>
              {answerList.map((answer, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{answer.answer}</td>
                  <td>{answer.questions ? <Link to={`questions/${answer.questions.id}`}>{answer.questions.id}</Link> : ''}</td>
                  <td>{answer.formAnswer ? <Link to={`form-answer/${answer.formAnswer.id}`}>{answer.formAnswer.id}</Link> : ''}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="studiesJHipsterApp.answer.home.notFound">No Answers found</Translate>
            </div>
          )
        )}
      </div>
