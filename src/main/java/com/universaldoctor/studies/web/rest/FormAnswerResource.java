...
/**
 * REST controller for managing {@link com.universaldoctor.studies.domain.FormAnswer}.
 */
@RestController
@RequestMapping("/api")
public class FormAnswerResource {
    ...
    public ResponseEntity<FormAnswer> updateFormAnswer(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, formAnswer.getId()))
            .body(result);
    }
     ...
     * @throws URISyntaxException if the Location URI syntax is incorrect.
     */
    @PatchMapping(value = "/form-answers/{id}", consumes = "application/merge-patch+json")
    public ResponseEntity<FormAnswer> partialUpdateFormAnswer(
        ...
        Optional<FormAnswer> result = formAnswerRepository
            ...
            .map(
                existingFormAnswer -> {
                    if (formAnswer.getCreationForm() != null) {
                        existingFormAnswer.setCreationForm(formAnswer.getCreationForm());
                    }
                    if (formAnswer.getModifiedForm() != null) {
                        existingFormAnswer.setModifiedForm(formAnswer.getModifiedForm());
                    }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, formAnswer.getId())
        );
    }
    ...
    public ResponseEntity<FormAnswer> getFormAnswer(@PathVariable String id) {
